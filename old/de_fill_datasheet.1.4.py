...
def comment_problem(ticket):
    ...
    if ticket.sheet_name == "dbdb" and ticket.image_pilot_version[:3] == "1.7":
        ticket.comment = ticket.comment + " Incorrect ImagePilot Version."
    if ticket.sheet_name == "dbdb9a" and ticket.image_pilot_version[:3] != "1.7":
        ticket.comment = ticket.comment + " Incorrect ImagePilot Version."
...
def rewrite_commented_tab(file, tickets):
    ...
    rownum_comment = 2
    tickets = sorted(tickets, key=attrgetter('ticket_close_date'), reverse=False)
...
def comment_out(tickets):
    for ticket in tickets:
        if type(ticket.ticket_close_date) is not datetime.datetime:
            if isinstance(ticket.last_db_access_date, basestring) and len(ticket.ticket_close_date) < 1:
