...
def fill_services_list(siteid):
    ...
    services = try_ldap(SERVICE_CONTRACTS, "(contractFlag=*)", "infomityServiceCode")
...
def fill_new_tickets(tickests):
    for ticket in tickests:
        ...
        #print len(ticket.services)
        for service in ticket.services:
            if service.service_name == "DATABANK":
                if service.contract_end[:4] != "2017" and service.contract_end[:4] != "2016":
                    print ticket.customer_dbid + " Contract start: " + service.contract_start + " End: " + service.contract_end
...
def fill_row(workbook, sheet_name, last_row):
	
    wb = copy(workbook)
    worksheet = wb.get_sheet(2)
    ...
    borders.right = xlwtd.Borders.THIN
    style.borders = borders
	
    worksheet.write(last_row + 11, 3, "Uraaaaaaaaaaaa", style=style)
    wb.save(EXCELFILE2)
...
common_list = assemply_trouble_list(dbst_new, dbdblist, dbdb9alist)
...
#fill_row(workbook, 'dbdb', get_last_row_number('dbdb', 11))
