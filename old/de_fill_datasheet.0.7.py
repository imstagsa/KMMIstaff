...
def write_new_tickets(workbook, tickets):
    ...
    for ticket in tickets:	
        if ticket.image_pilot_version[:3] == "1.7":
            worksheet_dbdb9a.write(last_row_dbdb9a, 3, d.strftime('%m/%d/%Y'), style=style)
            worksheet_dbdb9a.write(last_row_dbdb9a, 4, "Evghenii", style=style)
            worksheet_dbdb9a.write(last_row_dbdb9a, 5, ticket.customer_name, style=style)
            worksheet_dbdb9a.write(last_row_dbdb9a, 6, ticket.customer_dbid, style=style)
    
            if len(ticket.services) > 0:
                d1 = str_to_date4(ticket.services[0].service_end[:7])
                worksheet_dbdb9a.write(last_row_dbdb9a, 8, d1.strftime('%m/%d/%Y'), style=style)
                d1 = str_to_date4(ticket.services[0].contract_start[:7])
                worksheet_dbdb9a.write(last_row_dbdb9a, 9, d1.strftime('%m/%d/%Y'), style=style)
            last_row_dbdb9a += 1
        else:
            last_row_dbdb +=1
            worksheet_dbdb.write(last_row_dbdb, 3, d.strftime('%m/%d/%Y'), style=style)
            worksheet_dbdb.write(last_row_dbdb, 4, "Evghenii", style=style)
            worksheet_dbdb.write(last_row_dbdb, 5, ticket.customer_name, style=style)
            worksheet_dbdb.write(last_row_dbdb, 6, ticket.customer_dbid, style=style)
    
            if len(ticket.services) > 0:
                d1 = str_to_date4(ticket.services[0].service_end[:7])
                worksheet_dbdb.write(last_row_dbdb, 8, d1.strftime('%m/%d/%Y'), style=style)
                d1 = str_to_date4(ticket.services[0].contract_start[:7])
                worksheet_dbdb.write(last_row_dbdb, 9, d1.strftime('%m/%d/%Y'), style=style)
            last_row_dbdb += 1
...
def get_client_version(server, siteid):
    #version = ""
    ssh = paramiko.SSHClient()
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    ssh.connect(server, username='root', password='konicaminolta')
    cmd = "sh /infomity-mainte/bin/stcheck/KimSystemini_chk.sh | grep " + siteid
    stdin, stdout, stderr = ssh.exec_command(cmd)
    teststr = stdout.readlines()
    for x in range(len(teststr)):
        str1 = re.split(",", teststr[x])
...
def fill_and_write_new_tickets(tickets):
    for ticket in tickets:
        ...
        #print CUSTOMER_NAME
        str = try_ldap2(CUSTOMER_NAME, "(objectClass=*)", "medicalInstitutionName")
        if len(str) > 0:
            ticket.customer_name = str[0]
            #print str[0]
			
        TEL_NUMBER="nettsInstitutionCode=" + ticket.customer_dbid[:7] + ",o=institutions,dc=infomity,dc=net"
        str = try_ldap2(TEL_NUMBER,"(objectClass=*)", "telephoneNumber")
        if len(str) > 0:
        ...
        ticket.image_pilot_version = get_client_version(DBSTSRV, ticket.customer_dbid[:7])
...
#fill_and_write_new_tickets(dbdb_new)
